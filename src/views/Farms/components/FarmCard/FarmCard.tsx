...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethPrice, account }) => {
  ...
  const lpLabel = farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('ZONO', '')
  const earnLabel = farm.dual ? farm.dual.earnLabel : 'ZONO'
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'ZONO' && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
